    ...
    Attempt.successful(binaryProtocolValue.values.foldLeft(BitVector.empty) {
      case (acc, tuple) =>
        tuple match
          ...
          case (ColumnDataType.MYSQL_TYPE_SHORT | ColumnDataType.MYSQL_TYPE_YEAR, value: Short) => acc ++ uint16L.encode(value).require
          case (ColumnDataType.MYSQL_TYPE_LONG | ColumnDataType.MYSQL_TYPE_INT24, value: Int) => acc ++ uint32L.encode(value).require
          ...
          case (ColumnDataType.MYSQL_TYPE_STRING | ColumnDataType.MYSQL_TYPE_VARCHAR | ColumnDataType.MYSQL_TYPE_ENUM | ColumnDataType.MYSQL_TYPE_SET | ColumnDataType.MYSQL_TYPE_LONG_BLOB | ColumnDataType.MYSQL_TYPE_MEDIUM_BLOB | ColumnDataType.MYSQL_TYPE_BLOB | ColumnDataType.MYSQL_TYPE_TINY_BLOB | ColumnDataType.MYSQL_TYPE_GEOMETRY | ColumnDataType.MYSQL_TYPE_BIT | ColumnDataType.MYSQL_TYPE_DECIMAL | ColumnDataType.MYSQL_TYPE_NEWDECIMAL, value: String) =>
            val bytes = value.getBytes("UTF-8")
            acc ++ BitVector(copyOf(bytes, bytes.length))
          case (ColumnDataType.MYSQL_TYPE_VAR_STRING, value: Array[Byte]) => acc ++ BitVector(copyOf(value, value.length))
