...
package object packet:
  ...
  def time: Decoder[java.time.LocalTime] =
    for
      ...
      microsecond <- uint32L
    yield
      java.time.LocalTime.of(hour, minute, second, microsecond.toInt * 1000)

  def timestamp(length: Int): Decoder[java.time.LocalDateTime] =
    bytes(length).asDecoder.map(_.decodeUtf8 match
      case Left(value) => throw value
      case Right(value) => java.time.LocalDateTime.parse(value, java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))
    )
