...
object InitialPacket:
  ...
  private val authPluginDataPart1Codec: Codec[(Byte, Byte, Byte, Byte, Byte, Byte, Byte, Byte)] = byte :: byte :: byte :: byte :: byte :: byte :: byte :: byte
  ...
  val decoder: Decoder[InitialPacket] =
    for
      ...
      authPluginName <- if (capabilityFlags & (1 << 19)) != 0 then nullTerminatedStringCodec.asDecoder else Decoder.pure("")
    yield
      val capabilityFlags = (capabilityFlagsUpper << 16) | capabilityFlagsLower
      InitialPacket(protocolVersion, serverVersion, threadId, capabilityFlags, authPluginDataPart1 ++ authPluginDataPart2.toArray.dropRight(1), authPluginName)
